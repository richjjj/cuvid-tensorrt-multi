...
static void test_multi_decode()
{
    ...
    auto yolo = get_yolo(YoloGPUPtr::Type::V5, TRT::Mode::FP32, "yolov5s_pose", yolo_device_id);
    if (yolo == nullptr)
    {
    ...
    ts.emplace_back(std::bind(func, decoder->make_view("exp/34.mp4")));
    ts.emplace_back(std::bind(func, decoder->make_view("exp/35.mp4")));
    ts.emplace_back(std::bind(func, decoder->make_view("exp/36.mp4")));
    ts.emplace_back(std::bind(func, decoder->make_view("exp/37.mp4")));
    ts.emplace_back(std::bind(func, decoder->make_view("exp/38.mp4")));
    ts.emplace_back(std::bind(func, decoder->make_view("exp/39.mp4")));
    ts.emplace_back(std::bind(func, decoder->make_view("exp/37.mp4")));
    ts.emplace_back(std::bind(func, decoder->make_view("exp/35.mp4")));
    for (auto &t : ts)
        t.join();
