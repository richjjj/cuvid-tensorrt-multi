...
void render_to_images(vector<shared_future<YoloGPUPtr::BoxArray>> &objs_array, const string &name)
{
...
static void test_hard_decode()
{
    ...
    do
    {
        ...
        for (int i = 0; i < ndecoded_frame; ++i)
        {
            ...
            // cv::Mat img(decoder->get_height(), decoder->get_width(), CV_8UC3, decoder->get_frame(&pts, &frame_index));
            // cv::imwrite("r.jpg", img);
            // YoloGPUPtr::Image image(img);
