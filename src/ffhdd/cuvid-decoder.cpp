    ...
    public:
        ...
        int handlePictureDisplay(CUVIDPARSERDISPINFO *pDispInfo)
        {
            ...
            if (m_output_type == FrameType::BGR || m_output_type == FrameType::RGB)
            {
                ...
                uint8_t *uv = y + m_nWidth * m_nBPP * m_nLumaHeight;
                if (m_output_type == FrameType::BGR)
                    convert_nv12_to_bgr_invoker(y, uv, m_nWidth, m_nLumaHeight, (m_nWidth + 15) / 16 * 16, (uint8_t *)m_pBGRFrame, m_cuvidStream);
                else if (m_output_type == FrameType::RGB)
                    convert_nv12_to_rgb_invoker(y, uv, m_nWidth, m_nLumaHeight, (m_nWidth + 15) / 16 * 16, (uint8_t *)m_pBGRFrame, m_cuvidStream);
