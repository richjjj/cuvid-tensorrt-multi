...
std::shared_ptr<std::vector<uint8_t>> InferImpl::serial_engine() {
    ...
    auto output =
        make_shared<std::vector<uint8_t>>((uint8_t*)memory->data(), (uint8_t*)memory->data() + memory->size());
    memory->destroy();
    return output;
}
...
int InferImpl::get_max_batch_size() {
    ...
    return this->context_->engine_->getProfileDimensions(0, 0, nvinfer1::OptProfileSelector::kMAX).d[0];
}
