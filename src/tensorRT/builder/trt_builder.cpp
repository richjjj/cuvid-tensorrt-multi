    ...
    switch (layer->getType()) {
        case nvinfer1::LayerType::kCONVOLUTION: {
            ...
            return format("channel: %d, kernel: %s, padding: %s, stride: %s, dilation: %s, group: %d",
                          conv->getNbOutputMaps(), dims_str(conv->getKernelSizeNd()).c_str(),
                          dims_str(conv->getPaddingNd()).c_str(), dims_str(conv->getStrideNd()).c_str(),
                          dims_str(conv->getDilationNd()).c_str(), conv->getNbGroups());
        }
        case nvinfer1::LayerType::kFULLY_CONNECTED: {
        ...
        case nvinfer1::LayerType::kPOOLING: {
            nvinfer1::IPoolingLayer* pool = (nvinfer1::IPoolingLayer*)layer;
            return format("window: %s, padding: %s", dims_str(pool->getWindowSizeNd()).c_str(),
                          dims_str(pool->getPaddingNd()).c_str());
        }
        case nvinfer1::LayerType::kDECONVOLUTION: {
            nvinfer1::IDeconvolutionLayer* conv = (nvinfer1::IDeconvolutionLayer*)layer;
            return format("channel: %d, kernel: %s, padding: %s, stride: %s, group: %d", conv->getNbOutputMaps(),
                          dims_str(conv->getKernelSizeNd()).c_str(), dims_str(conv->getPaddingNd()).c_str(),
                          dims_str(conv->getStrideNd()).c_str(), conv->getNbGroups());
        }
        case nvinfer1::LayerType::kACTIVATION:
        case nvinfer1::LayerType::kPLUGIN:
        case nvinfer1::LayerType::kLRN:
...
std::string ModelSource::onnxmodel() const {
    return this->onnxmodel_;
}
ModelSourceType ModelSource::type() const {
    return this->type_;
}
...
void CompileOutput::set_data(const std::vector<uint8_t>& data) {
    data_ = data;
}

void CompileOutput::set_data(std::vector<uint8_t>&& data) {
    data_ = std::move(data);
}
/////////////////////////////////////////////////////////////////////////////////////////
...
public:
    ...
    Int8EntropyCalibrator(const vector<uint8_t>& entropyCalibratorData, nvinfer1::Dims dims,
...
bool compile(Mode mode, unsigned int maxBatchSize, const ModelSource& source, const CompileOutput& saveto,
             std::vector<InputDims> inputsDimsSetup, Int8Process int8process, const std::string& int8ImageDirectory,
    ...
    if (mode == Mode::INT8) {
            ...
            if (iLogger::exists(int8EntropyCalibratorFile)) {
                ...
                if (entropyCalibratorData.empty()) {
                    INFOE("entropyCalibratorFile is set as: %s, but we read is empty.",
                          int8EntropyCalibratorFile.c_str());
                    return false;
                }
        ...
        if (hasEntropyCalibrator) {
            ...
            if (entropyCalibratorFiles.size() < maxBatchSize) {
                INFOW("Too few images provided, %d[provided] < %d[max batch size], image copy will be performed",
                      entropyCalibratorFiles.size(), maxBatchSize);
    ...
    if (source.type() == ModelSourceType::OnnX || source.type() == ModelSourceType::OnnXData) {
        ...
        // network = shared_ptr<INetworkDefinition>(builder->createNetworkV2(explicitBatch),
        ...
        onnxParser.reset(nvonnxparser::createParser(*network, gLogger, dims_setup),
    ...
    for (int i = 0; i < net_num_layers; ++i) {
        ...
        INFOV("%s%s%s %s-> %s%s", token, number_str.c_str(), type_str.c_str(), input_dims_str.c_str(),
              output_dims_str.c_str(), descript.c_str());
    }
    ...
    // error on jetson
    // auto timing_cache = shared_ptr<nvinfer1::ITimingCache>(config->createTimingCache(nullptr, 0),
    // [](nvinfer1::ITimingCache* ptr){ptr->reset();}); config->setTimingCache(*timing_cache, false);
    ...
    shared_ptr<ICudaEngine> engine(builder->buildEngineWithConfig(*network, *config),
    ...
    if (saveto.type() == CompileOutputType::File) {
        ...
        ((CompileOutput&)saveto)
            .set_data(vector<uint8_t>((uint8_t*)seridata->data(), (uint8_t*)seridata->data() + seridata->size()));
        return true;
    }
}
};  // namespace TRT
