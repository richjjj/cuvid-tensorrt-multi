...
public:
    virtual bool startup(const string& engine_file, Type type, const vector<int> gpuids, float confidence_threshold,
...
#pragma omp parallel for num_threads(infers_.size())
        for (int i = 0; i < gpuids.size(); ++i) {
            auto& gpuid = gpuids[i];
            infers_[i]  = Yolo::create_infer(engine_file, type, gpuid, confidence_threshold, nms_threshold, nms_method,
                                             max_objects);
        }
...
shared_ptr<MultiGPUInfer> create_multi_gpu_infer(const string& engine_file, Type type, const vector<int> gpuids,
                                                 float confidence_threshold, float nms_threshold, NMSMethod nms_method,
                                                 int max_objects) {
