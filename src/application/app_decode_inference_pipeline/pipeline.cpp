                ...
                if (callback_) {
                    ...
                    for (int i = 0; i < tracks.size(); i++) {
                        ...
                        nlohmann::json event_json = {
                            ...
                            {"event", event_string},
                            {"box",
                             {track._tlwh[0], track._tlwh[1], track._tlwh[2] + track._tlwh[0],
                              track._tlwh[3] + track._tlwh[1]}},
                        ...
                        // cv::Point(track.tlwh[2] + track.tlwh[0], track.tlwh[3] + track.tlwh[1]), cv::Scalar(255, 0,
                        ...
                        // cv::Point(obj.left - 3, obj.top - 33), cv::Point(obj.left + width, obj.top), cv::Scalar(b, g,
                        // r), -1);
                    }
                    cv::Mat cvimage(image.get_height(), image.get_width(), CV_8UC3);
                    cudaMemcpyAsync(cvimage.data, image.device_data, image.get_data_size(), cudaMemcpyDeviceToHost,
