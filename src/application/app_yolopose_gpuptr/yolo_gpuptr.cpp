...
using ControllerImpl = InferController<Image,               // input
                                       BoxArray,            // output
...
public:
    ...
    virtual void worker(promise<bool> &result) override {
        ...
        const int NUM_BOX_ELEMENT = 7 + 51;  // left, top, right, bottom, confidence, class, keepflag + 17*3
            ...
            for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch) {
                ...
                for (int i = 0; i < count; ++i) {
                    ...
                    if (keepflag == 1) {
                        ...
                        memcpy(box.pose, pbox + 7, sizeof(box.pose));
    ...
    virtual bool preprocess(Job &job, const Image &image) override {
        ...
        if (tensor->device() != image.device_id) {
            ...
            checkCudaRuntime(cudaMemcpyPeerAsync(pimage_device, tensor->device(), image.device_data, image.device_id,
        ...
        CUDAKernel::warp_affine_bilinear_and_normalize_plane(
            image_device, image.get_width() * 3, image.get_width(), image.get_height(), tensor->gpu<float>(),
            input_width_, input_height_, affine_matrix_device, 114, normalize_, preprocess_stream);
...
shared_ptr<Infer> create_infer(const string &engine_file, Type type, int gpuid, float confidence_threshold,
                               float nms_threshold, NMSMethod nms_method, int max_objects) {
    shared_ptr<InferImpl> instance(new InferImpl());
    if (!instance->startup(engine_file, type, gpuid, confidence_threshold, nms_threshold, nms_method, max_objects)) {
        instance.reset();
    }
    return instance;
}
