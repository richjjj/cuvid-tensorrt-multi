...
const int NUM_BOX_ELEMENT = 7 + 51;  // left, top, right, bottom, confidence, class, keepflag
...
static __global__ void decode_kernel(float *predict, int num_bboxes, int num_classes, float confidence_threshold,
    ...
    float *pitem     = predict + (6 + 51) * position;
    ...
    *pout_item++     = 1;  // 1 = keep, 0 = ignore
    ...
    for (int i = 0; i < 17; i++) {
        float x = landmark_predict[i];
        float y = landmark_predict[i + 17];
        affine_project(invert_affine_matrix, x, y, pout_item + 0, pout_item + 1);
        pout_item[2] = landmark_predict[i + 17 * 2];
        pout_item += 3;
    }
}
